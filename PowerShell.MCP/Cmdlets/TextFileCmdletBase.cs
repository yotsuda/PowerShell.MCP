...
public abstract class TextFileCmdletBase : PSCmdlet
{
        ...
        IEnumerable<int> matchedLines, int maxLine, int contextLines = 2)
    {
        ...
        // 各マッチの前後N行の範囲を計算
        // matchedLinesは昇順なので、rangesも自動的にソート済み
        ...
        // 重複または隣接する範囲をマージ（ギャップ2行以下もマージし、ギャップ行を記録）
        var merged = new List<(int start, int end)>();
        var current = ranges[0];

        for (int i = 1; i < ranges.Count; i++)
        {
            int gapSize = ranges[i].start - current.end - 1;
            
            if (gapSize <= 1)
            {
                // 範囲が重複、隣接、または1行ギャップ → マージ
                // ギャップ行を記録
                for (int gapLine = current.end + 1; gapLine < ranges[i].start; gapLine++)
                {
