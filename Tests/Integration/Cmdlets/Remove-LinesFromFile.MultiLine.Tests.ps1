...
"@
    ...
    Context "Multi-line regex pattern removal" {
        It "Should remove multi-line pattern match with -Pattern" {
            $testFile = [System.IO.Path]::GetTempFileName()
            try {
                @"
Start
...
"@ | Set-Content $testFile
                ...
                Remove-LinesFromFile -Path $testFile -Pattern $pattern

                $result = Get-Content $testFile -Raw
                $result | Should -Match "Start"
                $result | Should -Match "Final"
                $result | Should -Not -Match "Line A:"
                $result | Should -Not -Match "Line B:"
                $result | Should -Not -Match "Line C:"
            }
            finally {
                if (Test-Path $testFile) { Remove-Item $testFile -Force }
            }
...
"@
                ...
                $output | Should -Match "1 occurrence"
                $output | Should -Match "Line 1"
                $output | Should -Match "Line 4"
                
                # Verify removed lines shown without line numbers
                $output | Should -Match "   :"
...
"@
    ...
    Context "Backup support" {
        It "Should create backup when -Backup specified" {
            $testFile = [System.IO.Path]::GetTempFileName()
            try {
                @"
Line 1
...
"@
                ...
                $output = Remove-LinesFromFile -Path $testFile -Contains $searchText 2>&1 | Out-String
        ...
        It "Should not support tail removal with multi-line patterns" {
            $testFile = [System.IO.Path]::GetTempFileName()
            try {
