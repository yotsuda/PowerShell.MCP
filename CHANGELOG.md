...
- **`Get-MCPOwner` no longer reports a Claude Code session as `Claude Desktop`.** Both are a process named `claude` — `claude.exe` on Windows, `Claude` on macOS — and the client was identified by process name before path, so the Desktop branch swallowed every Claude Code session. (It only began to: the CLI used to be an npm install and matched on `node`, until it became a standalone binary.) The install path now decides — `...\AnthropicClaude\...` or `/Applications/Claude.app/...` is Desktop, a standalone `claude` is the CLI. This value is displayed, never branched on; the timeout ceiling above identifies the client from the MCP handshake instead, which is why it was unaffected.
